...
class SelectCountriesFrame(tk.Frame):
    def __init__(self, master):
        ...
        tk.Label(self, text='Select Countries from list below',
                 font=('Arial Bold', 18)).pack(side=tk.TOP)
        self.countries = TTK.Combobox(
            self, width=27, textvariable=self.count_var)
        ...
        # create buttons
        tk.Button(self, text="Back", command=lambda: self.back(
            master)).pack(side=tk.TOP)
        tk.Button(self, text="Add Country to List",
                  command=lambda: self.addToSelectedList(master)).pack(side=tk.TOP)
        tk.Button(self, text="Next", command=lambda: self.next(
            master)).pack(side=tk.TOP)

    def next(self, master):
        import imf_select_indicators_frame
        if len(self.select_list) == 0:
            ...
            master.switch_frame(
                imf_select_indicators_frame.SelectIndicatorsFrame)

    def back(self, master):
        import imf_select_frequency_frame
        master.switch_frame(imf_select_frequency_frame.SelectFrequencyFrame)
