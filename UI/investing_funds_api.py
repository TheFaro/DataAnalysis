...
class FundsAPI:
    ...
    def getFundData(self):
        try:
            data = investpy.get_fund_historical_data(
                fund=self.fund, country=self.country, from_date=self.from_date, to_date=self.to_date, as_json=True)
            return data
    ...
    def getRecentFundData(self):
        try:
            data = investpy.get_fund_recent_data(
                fund=self.fund, country=self.country)
            return data
        except (ValueError, FileNotFoundError, IOError, RuntimeError, ConnectionError):
            if ValueError:
                mb.showinfo(
