...
class IMFDataProcessor:
    ...
    def getIMFDataFromServer(self):
        ...
        if self.indicator != None:
            for i, country in enumerate(self.countries):
                ...
                try:
                    result = requests.get(f"{const.server}/imf/get/{self.indicator}/{country['code']}",headers=const.headers)
    ...
    def saveDataInServer(self, series):
        ...
        result = requests.post(f"{const.server}/imf/create", data=json.dumps(data),headers=const.headers)
