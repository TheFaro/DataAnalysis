...
class MacrotrendsMenu(tk.Frame):
    def __init__(self, master):
        ...
        # build title
        row = tk.Frame(self)
        lab = tk.Label(row, text='Macrotrends Menu',
        ...
        row = tk.Frame(self)
        but = tk.Button(row, text='Update Data', width=20,
                        font=self.const.font, command=lambda: self.updateData(master))
        row.pack(pady=20)
        but.pack()
        ...
        row = tk.Frame(self)
        but = tk.Button(row, text='Plot Data', width=20, font=self.const.font,
                        command=lambda: master.switch_frame(PlotTrendsDataFrame))
        row.pack(pady=20)
        but.pack()
    ...
    def goBack(self, master):
        ...
        master.switch_frame(choose_data_source_frame.ChooseDataSourceFrame)
...
class PlotTrendsDataFrame(tk.Frame):
    def __init__(self, master):
        ...
        lab = tk.Label(row, text='Select Plot Options',
                       font=self.self.const.title_font)
        row.pack(pady=15)
        lab.pack()
        ...
        self.opts = ['Overview', 'Descriptive', 'Dividends', 'Performance Short Term',
                     'Performance Long Term', 'Income Ratios', 'Debt Ratios', 'Revenue Earnings']

        for i, opt in enumerate(self.opts):
            row = tk.Frame(self.frame.scrollable_frame)
            but = tk.Button(row, text=opt, font=self.self.const.font,
                            width=30, command=lambda i=i: self.optionClick(i))
            row.pack(pady=5)
            but.pack()
    ...
    def optionClick(self, index):
        ...
        result = requests.post(f'{self.const.server}/macrotrends/columns/get',
                               data=json.dumps(data), headers=self.self.const.headers).json()
        ...
        if result['success']:
            ...
            for i, col in enumerate(self.col_buts):
                row = tk.Frame(self.frame.scrollable_frame)
                but = tk.Button(row, text=col, font=self.self.const.font,
                                command=lambda i=i: self.columnClick(i))
                row.pack(pady=5)
                but.pack()
        else:
    ...
    def columnClick(self, index):
        ...
        result = requests.get(
            f'{self.const.server}/macrotrends/overview/get/all', headers=self.self.const.headers).json()
        print("Result: \n", result)

        # put the stock names in a combobox
        row = tk.Frame(self.frame.scrollable_frame)
        lab = tk.Label(row, text='Select Stocks', font=self.self.const.font)
        ...
        row = tk.Frame(big_row)
        hint = tk.Label(
            row, text='hint: double click on list item to remove from list')
        row.pack(pady=15)
        hint.pack()
        big_row.pack()
        ...
        row = tk.Frame(self.frame.scrollable_frame)
        but = tk.Button(row, text='Plot Button', width=20,
                        font=self.self.const.font, command=lambda: self.plotData(self.master))
        row.pack()
        but.pack()
