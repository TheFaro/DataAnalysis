...
class PlotCryptosFrame(tk.Frame):
    ...
    def plotData(self, master):
        ...
        result = requests.get(
            f"{self.const.server}/investing/crypto/get/{self.api.getName()}_{self.api.getCountry()}/{self.start_date.get().replace('/','')}/{self.end_date.get().replace('/','')}", headers=self.const.headers).json()
