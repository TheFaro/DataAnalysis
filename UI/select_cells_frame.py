...
class SelectCellsFrame(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.frame = scrollable_frame.ScrollableFrame(master)
    ...
    def buildWidgets(self, master):
        ...
        lab = tk.Label(row, text='Select Sheet cells to be plotted',
                       font=('Arial', 17, 'bold'))
        ...
        lab = tk.Label(row, text='Select One table',
                       font=('Arial', 17, 'bold'))
        hint = tk.Label(row, text='(enter the top most left cell and the bottom most right)', font=(
            'Arial', 12, 'italic'))
        row.pack(side='top', expand='yes', pady=20)
        tk.Radiobutton(row, text='', value='Table',
                       variable=self.var).pack(side='left')
        lab.pack(side='top')
        hint.pack(side='top')

        # top left cell
        row = tk.Frame(self.frame.scrollable_frame)
        lab = tk.Label(row, text='Top Left Cell (i.e. A10)', font=(
            'Arial', 15, 'bold'), relief='ridge', width='30')
        self.top_left_cell = tk.Entry(row, font=(
            'Arial', 15, 'bold'), width=20, relief='sunken')
        ...
        # bottom right cell
        row = tk.Frame(self.frame.scrollable_frame)
        lab = tk.Label(row, text='Bottom Right Cell (i.e. F43)', font=(
            'Arial', 15, 'bold'), relief='ridge', width='30')
        self.bottom_right_cell = tk.Entry(row, font=(
            'Arial', 15, 'bold'), width=20, relief='sunken')
        ...
        row = tk.Frame(self.frame.scrollable_frame)
        lab = tk.Label(row, text='Select multiple columns to plot',
                       font=('Arial', 17, 'bold'))
        ...
        tk.Radiobutton(row, text='', value='Columns',
                       variable=self.var).pack(side='left')
        lab.pack(side='top')
        # hint.pack(side='top')
        ...
        big_row = tk.Frame(self.frame.scrollable_frame)
        row = tk.Frame(big_row)
        lab = tk.Label(row, text='Columns', font=(
            'Arial', 15, 'bold'), width=30, relief='ridge')
        self.multiple_columns = tk.Entry(row, font=(
            'Arial', 15, 'bold'), width=20, relief='sunken')
        row.pack(side='top')
        lab.pack(side='left')
        ...
        row = tk.Frame(big_row)
        hint = tk.Label(row, text='(List the letters of the columns desired (i.e. ABEF))', font=(
            'Arial', 12, 'italic'))
        row.pack(side='top')
        hint.pack(side='top')
        big_row.pack(side='top')
        ...
        # start row number
        row = tk.Frame(self.frame.scrollable_frame)
        lab = tk.Label(row, text='Start Row Number', font=(
            'Arial', 15, 'bold'), relief='ridge', width=30)
        self.start_row = tk.Entry(row, width=20, font=(
            'Arial', 15, 'bold'), relief='sunken')
        row.pack(side='top')
        lab.pack(side='left')
        ...
        big_row = tk.Frame(self.frame.scrollable_frame)
        row = tk.Frame(big_row)
        lab = tk.Label(row, text='End Row Number', font=(
            'Arial', 15, 'bold'), relief='ridge', width=30)
        self.end_row = tk.Entry(row, width=20, font=(
            'Arial', 15, 'bold'), textvariable=tk.StringVar(row, value='0'), relief='sunken')
        row.pack(side='top')
        lab.pack(side='left')
        self.end_row.pack(side='right')

        row = tk.Frame(big_row)
        hint = tk.Label(row, text='(If empty, data will be read until the end of the column)', font=(
            'Arial', 12, 'italic'))
        row.pack(side='top')
        hint.pack(side='top')
        ...
        # x-axis name/ label
        row = tk.Frame(self.frame.scrollable_frame)
        lab = tk.Label(row, text='x-axis label', font=('Arial',
                       15, 'bold'), width=30, relief='ridge')
        self.x_axis_label = tk.Entry(row, width=20, font=(
            'Arial', 15, 'bold'), relief='sunken')
        ...
        # y-axis label
        row = tk.Frame(self.frame.scrollable_frame)
        lab = tk.Label(row, text='y-axis label', font=('Arial',
                       15, 'bold'), width=30, relief='ridge')
        self.y_axis_label = tk.Entry(row, width=20, font=(
            'Arial', 15, 'bold'), relief='sunken')
        row.pack(side='top', pady=15)
        lab.pack(side='left')
        ...
        row = tk.Frame(self.frame.scrollable_frame)
        lab = tk.Label(row, text='x-axis column name',
                       font=('Arial', 15, 'bold'), width=30, relief='ridge')
        self.x_name = tk.Entry(row, width=20, font=(
            'Arial', 15, 'bold'), relief='sunken')
        row.pack(side='top')
        lab.pack(side='left')
        ...
        row = tk.Frame(self.frame.scrollable_frame)
        but = tk.Button(row, text='Plot', font=('Arial', 15, 'bold'),
                        width=20, command=lambda: self.plotGraph())
        row.pack(side='top', pady=15)
        but.pack(side='top')

        # button to select new sheet
        row = tk.Frame(self.frame.scrollable_frame)
        but = tk.Button(row, text='Select New Sheet', font=(
            'Arial', 15, 'bold'), width=20, command=lambda: self.selectSheet(master))
        row.pack(side='top', pady=15)
        but.pack(side='top')

        # button to select a different file
        row = tk.Frame(self.frame.scrollable_frame)
        but = tk.Button(row, text='Select New File', font=(
            'Arial', 15, 'bold'), width=20, command=lambda: self.selectFile(master))
        row.pack(side='top', pady=15)
        but.pack(side='top')
    ...
    def selectSheet(self, master):
        ...
        master.switch_frame(select_sheet_frame.SelectSheet)
    ...
    def plotGraph(self):
        ...
        if self.var.get() == 'Table':
            ...
            if int(self.end_row.get()) == 0:
                ...
                for row in range(int(self.start_row.get()), len(list(self.ws.rows))+1):
                    ...
                    for column in self.multiple_columns.get():
                        data_cols.append(
                            self.ws["{}{}".format(column, row)].value)
                    data_row.append(data_cols)
            ...
            mb.showinfo(
                'Notice', 'Please choose one of the two options: Select One Table or Select multiple columns')
            return
