...
class SearchForSeriesDataflow(tk.Frame):
    def __init__(self, master):
        ...
        tk.Label(self, text='Select Indicator For Data Retrieval',
                 width="50").pack(side=tk.TOP)

        # finished building and rendering, create back button
        tk.Button(self, text='Back', command=lambda: self.goBack(
            master)).pack(side=tk.BOTTOM)
            ...
            if self.master.search_term == None:
                ...
                if code != None:
                    ...
                    self.codes_list.append(
                        {"name": f"{series['Name']['#text']}", "id": f"{series['KeyFamilyRef']['KeyFamilyID']}"})
                ...
                print(
                    f"{self.master.search_term} could not be found in the Database Structures")

        if self.master.search_term == None:
            tk.Label(self, text=f"Could not find search item : {self.master.search_term}").pack(
                side=tk.TOP, pady=15)
            ...
            tk.Label(self, text=f"Could not find {master.search_tem} in Database Structures").pack(
                side=tk.TOP, pady=10)
