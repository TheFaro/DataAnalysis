...
class PlotFundsFrame(tk.Frame):
    ...
    def __init__(self, master):
        ...
        row = tk.Frame(self)
        lab = tk.Label(row, text='Funds Plotting',font=const.title_font)
        row.pack()
        lab.pack()
        ...
        row = tk.Frame(self)
        lab = tk.Label(row, text='Select Fund Name', font=const.font)
    ...
    def plotData(self,master):
        ...
        result = requests.get(f"{const.server}/investing/funds/get/{self.api.getName()}_{self.api.getCountry()}/{self.start_date.get().replace('/','')}/{self.end_date.get().replace('/','')}",headers=const.headers).json()
    ...
    def updateData(self):
        ...
        recent = requests.get(f"{const.server}/investing/funds/get/recent",data=json.dumps(data), headers=const.headers)
        ...
        if recent.status_code == 404:
            ...
            const.saveDataInServer(dat, 'investing/funds', 'fund_name', self.api)
            ...
            mb.showinfo('Notice', f'Data on {self.api.getFund()} updated successfully.')
