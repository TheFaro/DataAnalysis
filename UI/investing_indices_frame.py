...
class PlotIndicesFrame(tk.Frame):
    def __init__(self, master):
        ...
        row = tk.Frame(self)
        lab = tk.Label(row, text='Indices Plotting',font=const.title_font)
        row.pack()
        lab.pack()
        ...
        row = tk.Frame(self)
        lab = tk.Label(row, text='Select Index',font=const.font)
    ...
    def plotData(self,master):
        ...
        result = requests.get(f"{const.server}/investing/indices/get/{self.api.getName()}_{self.api.getCountry()}/{self.start_date.get().replace('/','')}/{self.end_date.get().replace('/','')}",headers=const.headers).json()
    ...
    def updateData(self):
        ...
        recent = requests.get(f"{const.server}/investing/indices/get/recent",data=json.dumps(data), headers=const.headers)
        print(recent)
        ...
        if recent.status_code == 404:
            ...
            const.saveDataInServer(dat, 'investing/indices', 'index_name', self.api)
